...
function main()
    ...
    bindFastActiveUgenrl = rkeys.registerHotKey(FastUgenrlKey.v, true, function()
        if not sampIsCursorActive() then
            ...
            sampAddChatMessage(ini.ugenrl_main.enable and script_name.." {FFFFFF}Ultimate Genrl {73b461}Switched On" or script_name.." {FFFFFF}Ultimate Genrl {dc4747}Switched Off", 0x73b461)
        end
    end)
    ...
    while true do
        ...
        if not sampIsCursorActive() then
            if ini.settings.chatt and testCheat("T") then
                sampSetChatInputEnabled(true)
            end
        end

        if ini.settings.antiblockedplayer == true and not isCharInAnyCar(PLAYER_PED) then
            for i = 0, 999 do
                if sampIsPlayerConnected(i) then
                    ...
                    if result then
                        if doesCharExist(id) then
                            ...
                            if 0.5 > getDistanceBetweenCoords3d(x, y, z, mX, mY, mZ) then
                                setCharCollision(id, true)
                            end
                        end
                    end
                end
...
function ev.onSendCommand(cmd)
    ...
    elseif cmd:find("^"..ini.commands.settime.." .+") or cmd:find("^"..ini.commands.settime.."$") then
        ...
        if ini.settings.blocktime then
            if type(hours) ~= 'number' or hours < 0 or hours > 23 or type(min) ~= 'number' or min < 0 or min > 59 then
                sampAddChatMessage(script_name.." {FFFFFF}Use: {dc4747}"..ini.commands.settime.." [0-23 - hours] [0-59 - min]", 0x73b461)
                sampAddChatMessage(script_name.." {FFFFFF}Current parameter: {dc4747}"..ini.settings.hours..":"..ini.settings.min, 0x73b461)
            else
                ini.settings.hours = hours
                ini.settings.min = min
                save()
                gotofunc("SetTime")
                sampAddChatMessage(script_name.." {FFFFFF}Time Set to: {dc4747}"..ini.settings.hours..":"..ini.settings.min, 0x73b461)
            end
        else
            sampAddChatMessage(script_name.." {FFFFFF}Time is Forzen, Enter: {dc4747}"..ini.commands.blockservertime, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.setweather.." .+") or cmd:find("^"..ini.commands.setweather.."$") then
        ...
        if ini.settings.blockweather then
            if type(weather) ~= 'number' or weather < 0 or weather > 45 then
                sampAddChatMessage(script_name.." {FFFFFF}Use: {dc4747}"..ini.commands.setweather.." [0-45]", 0x73b461)
                sampAddChatMessage(script_name.." {FFFFFF}Current parameter: {dc4747}"..ini.settings.weather, 0x73b461)
            else
                ini.settings.weather = weather
                save()
                gotofunc("SetWeather")
                sampAddChatMessage(script_name.." {FFFFFF}Weather Set to: {dc4747}"..ini.settings.weather, 0x73b461)
            end
        else
            sampAddChatMessage(script_name.." {FFFFFF}Weather is Frozen, Enter: {dc4747}"..ini.commands.blockserverweather, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.blockservertime.."$") then
        ini.settings.blocktime = not ini.settings.blocktime
        save()
        sampAddChatMessage(ini.settings.blocktime and script_name..' {FFFFFF}Time Has been {dc4747}UnFrozen ' or script_name..' {FFFFFF}Time has been {73b461}Frozen ', 0x73b461)
        return false
    elseif cmd:find("^"..ini.commands.blockserverweather.."$") then
        ini.settings.blockweather = not ini.settings.blockweather
        save()
        sampAddChatMessage(ini.settings.blockweather and script_name..' {FFFFFF}Weather has been {dc4747}Unfrozen ' or script_name..' {FFFFFF}Weather Has Been {73b461}Forzen ', 0x73b461)
        return false
    ...
    elseif cmd:find("^"..ini.commands.fogdistance.." .+") or cmd:find("^"..ini.commands.fogdistance.."$") then
        ...
        if ini.settings.givemedist then
            if type(fogdist) ~= 'number' or fogdist > 500 or fogdist < 0 then
    ...
    elseif cmd:find("^"..ini.commands.loddistance.." .+") or cmd:find("^"..ini.commands.loddistance.."$") then
        ...
        if ini.settings.givemedist then
            if type(loddist) ~= 'number' or loddist > 1000 or loddist < 0 then
                sampAddChatMessage(script_name.." {FFFFFF}Use: {dc4747}"..ini.commands.loddistance.." [0-1000]", 0x73b461)
    ...
    elseif cmd:find("^"..ini.commands.chatopenfix.."$") then
        ...
        save()
        sampAddChatMessage(ini.settings.chatt and script_name..' {FFFFFF}Chat Opening on E {73b461}Enabled' or script_name..' {FFFFFF}Chat Opening on E {dc4747}Disabled', 0x73b461)
        return false
    elseif cmd:find("^"..ini.commands.hphud.."$") then
        ...
        return false
    elseif cmd:find("^"..ini.commands.hpstyle.." .+") or cmd:find("^"..ini.commands.hpstyle.."$") then
        local hpstyle = cmd:match(ini.commands.hpstyle.." (.+)")
        hpstyle = tonumber(hpstyle)
        if type(hpstyle) ~= 'number' or hpstyle > 3 or hpstyle < 0 then
            sampAddChatMessage(script_name.." {FFFFFF}Use: {DC4747}"..ini.commands.hpstyle.." [0-3]", 0x73b461)
        else
            ini.hphud.text = hpstyle
            save()
            sampAddChatMessage(script_name.." {FFFFFF}Health Font Style Set: {DC4747}"..ini.hphud.text, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.hppos.." .+") or cmd:find("^"..ini.commands.hppos.."$") then
        local hppos = cmd:match(ini.commands.hppos.." (.+)")
        hppos = tonumber(hppos)
        if type(hppos) ~= 'number' or hppos > 3 or hppos < 1 then
            sampAddChatMessage(script_name.." {FFFFFF}Use: {DC4747}"..ini.commands.hppos.." {FFFFFF}- [1, 2, 3]", 0x73b461)
        else
            ini.hphud.pos = hppos
            save()
            sampAddChatMessage(script_name.." {FFFFFF}Hp Position set to: {DC4747}"..ini.hphud.pos.."", 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.hptext.."$") then
        ini.hphud.hpt = not ini.hphud.hpt
        hpt()
        save()
        return false
    elseif cmd:find("^"..ini.commands.autocleaner.."$") then
        ...
        save()
        sampAddChatMessage((script_name.."{FFFFFF} Automatic memory cleaning % s"):format(ini.cleaner.autoclean and "{73b461}Enabled" or "{dc4747}Disabled"), 0x73b461)
        return false
    ...
    elseif cmd:find("^"..ini.commands.cleaninfo.."$") then
        ...
        sampAddChatMessage(ini.cleaner.cleaninfo and script_name..' {FFFFFF}Message Memory Cleaning {73b461}Enabled' or script_name..' {FFFFFF}Message Memory Cleaning {dc4747}Disabled', 0x73b461)
        return false
    elseif cmd:find("^"..ini.commands.setmbforautocleaner.." .+") or cmd:find("^"..ini.commands.setmbforautocleaner.."$") then
        ...
        if type(setccl) ~= 'number' or setccl > 2000 or setccl < 0 then
            sampAddChatMessage(script_name.." {FFFFFF}Use: {dc4747}"..ini.commands.setmbforautocleaner.." [0-2000 МБ]", 0x73b461)
            sampAddChatMessage(script_name.." {FFFFFF}Current Parameter: {dc4747}"..ini.cleaner.limit.." МБ", 0x73b461)
        else
            ini.cleaner.limit = setccl
            save()
            sampAddChatMessage(script_name.." {FFFFFF}CCL is set to: {dc4747}"..ini.cleaner.limit.." MB", 0x73b461)
        end
        return false
    ...
    elseif cmd:find("^"..ini.commands.fixtimecyc.."$") then
        ...
        sampAddChatMessage(ini.fixtimecyc.active and script_name..' {FFFFFF}Timecycle Fix Is now {73b461}Enabled' or script_name..' {FFFFFF}Timecycle Fix Is now {dc4747}Disabled', 0x73b461)
        save()
        gotofunc("FixTimecyc")
        return false
    elseif cmd:find("^"..ini.commands.effects.."$") then
        ...
        save()
        sampAddChatMessage(ini.settings.effects and script_name..' {FFFFFF}Effects is now {73b461}Enabled' or script_name..' {FFFFFF}Effects is now {dc4747}Disabled', 0x73b461)
        gotofunc("NoEffects")
        return false
    ...
    elseif cmd:find("^"..ini.commands.shadowedit.."$") then
        ...
        save()
        sampAddChatMessage(ini.settings.shadowedit and script_name..' {FFFFFF}Ability to edit Shadows is now {73b461}Enabled' or script_name..' {FFFFFF}Ability to edit Shadows is now {dc4747}Disabled', 0x73b461)
        gotofunc("ShadowEdit")
        return false
    elseif cmd:find("^"..ini.commands.nocloudbig.."$") then
        ini.settings.nocloudbig = not ini.settings.nocloudbig
        save()
        sampAddChatMessage(ini.settings.nocloudbig and script_name..' {FFFFFF}NoBigClouds is now {73b461}Enabled' or script_name..' {FFFFFF}NobigClouds is now {dc4747}disabled', 0x73b461)
        gotofunc("NoCloudBig")
        return false
    elseif cmd:find("^"..ini.commands.nocloudsmall.."$") then
        ini.settings.nocloudsmall = not ini.settings.nocloudsmall
        save()
        sampAddChatMessage(ini.settings.nocloudsmall and script_name..' {FFFFFF}NoSmallClouds is now {73b461}Enabled' or script_name..' {FFFFFF}NoSmallClouds {dc4747}Disabled', 0x73b461)
        gotofunc("NoCloudSmall")
        return false
    elseif cmd:find("^"..ini.commands.noshadows.."$") then
        ini.settings.noshadows = not ini.settings.noshadows
        save()
        sampAddChatMessage(ini.settings.noshadows and script_name..' {FFFFFF}NoShadows is now {73b461}Enabled' or script_name..' {FFFFFF}NoShadows is now {dc4747}Disabled', 0x73b461)
        gotofunc("NoShadows")
        return false
    elseif cmd:find("^"..ini.commands.vehlods.."$") then
        ini.settings.vehlods = not ini.settings.vehlods
        save()
        sampAddChatMessage(ini.settings.vehlods and script_name..' {FFFFFF}VehLods is now {73b461}Enabled' or script_name..' {FFFFFF}VehLods is now {dc4747}Disabled', 0x73b461)
        gotofunc("VehLods")
        return false
    elseif cmd:find("^"..ini.commands.fixcrosshair.."$") then
        ini.settings.fixcrosshair = not ini.settings.fixcrosshair
        save()
        sampAddChatMessage(ini.settings.fixcrosshair and script_name..' {FFFFFF}Crosshair White Dot Fix is now  {73b461}Enabled' or script_name..' {FFFFFF}Crosshair White Dot Fix is now {dc4747}Disabled', 0x73b461)
        gotofunc("FixCrosshair")
        return false
    elseif cmd:find("^"..ini.commands.intrun.."$") then
        ini.settings.intrun = not ini.settings.intrun
        save()
        sampAddChatMessage(ini.settings.intrun and script_name..' {FFFFFF}InteriorRun is now {73b461}Enabled' or script_name..' {FFFFFF}InteriorRun is now {dc4747}Disabled', 0x73b461)
    ...
    elseif cmd:find("^"..ini.commands.waterfixquadro.."$") then
        ...
        save()
        sampAddChatMessage(ini.settings.waterfixquadro and script_name..' {FFFFFF}Square Water fix is now {73b461}Enabled' or script_name..' {FFFFFF}Square Water fix is now {dc4747}Disabled', 0x73b461)
        gotofunc("FixWaterQuadro")
        return false
    elseif cmd:find("^"..ini.commands.longarmfix.."$") then
        ini.settings.longarmfix = not ini.settings.longarmfix
        save()
        sampAddChatMessage(ini.settings.longarmfix and script_name..' {FFFFFF}LongArm fix is now {73b461}Enabled' or script_name..' {FFFFFF}LongArm fix is now {dc4747}Disabled', 0x73b461)
        gotofunc("FixLongArm")
        return false
    elseif cmd:find("^"..ini.commands.fixblackroads.."$") then
        ini.settings.fixblackroads = not ini.settings.fixblackroads
        save()
        sampAddChatMessage(ini.settings.fixblackroads and script_name..' {FFFFFF}BlackRoads fix is now {73b461}Enabled' or script_name..' {FFFFFF}BlackRoads fix is now {dc4747}Disabled', 0x73b461)
        gotofunc("FixBlackRoads")
        return false
    elseif cmd:find("^"..ini.commands.sensfix.."$") then
        ini.settings.sensfix = not ini.settings.sensfix
        save()
        sampAddChatMessage(ini.settings.sensfix and script_name..' {FFFFFF}Sensitivity fix is now {73b461}Enabled' or script_name..' {FFFFFF}Sensitivity fix is now {dc4747}Disabled', 0x73b461)
    ...
    elseif cmd:find("^"..ini.commands.audiostream.."$") then
        ...
        save()
        sampAddChatMessage(ini.settings.audiostream and script_name..' {FFFFFF}AudioStream {73b461}Enabled' or script_name..' {FFFFFF}AudioStream {dc4747}Disabled', 0x73b461)
        gotofunc("AudioStream")
        return false
    elseif cmd:find("^"..ini.commands.intmusic.."$") then
        ini.settings.intmusic = not ini.settings.intmusic
        save()
        sampAddChatMessage(ini.settings.intmusic and script_name..' {FFFFFF}Music in interiors {73b461}Enabled' or script_name..' {FFFFFF}Music in interiors {dc4747}Disabled', 0x73b461)
        gotofunc("InteriorMusic")
        return false
    elseif cmd:find("^"..ini.commands.nosounds.."$") then
        ini.settings.nosounds = not ini.settings.nosounds
        save()
        sampAddChatMessage(ini.settings.nosounds and script_name..' {FFFFFF}NoSounds is now {73b461}Enabled' or script_name..' {FFFFFF}NoSounds is now {dc4747}Disabled', 0x73b461)
        gotofunc("NoSounds")
        return false
    elseif cmd:find("^"..ini.commands.noplaneline.."$") then
        ini.settings.noplaneline = not ini.settings.noplaneline
        save()
        sampAddChatMessage(ini.settings.noplaneline and script_name..' {FFFFFF}NoPlaneLine {73b461}Enabled' or script_name..' {FFFFFF}NoPlaneLine {dc4747}Disabled', 0x73b461)
        gotofunc("NoPlaneLine")
        return false
    elseif cmd:find("^"..ini.commands.blockkeys.."$") then
        ini.settings.blockkeys = not ini.settings.blockkeys
        save()
        sampAddChatMessage(ini.settings.blockkeys and script_name..' {FFFFFF}Locking samp client keys {73b461}Enabled' or script_name..' {FFFFFF}Locking samp client keys {dc4747}Disabled', 0x73b461)
        gotofunc("BlockSampKeys")
        return false
    elseif cmd:find("^"..ini.commands.sunfix.."$") then
        ini.settings.sunfix = not ini.settings.sunfix
        save()
        sampAddChatMessage(ini.settings.sunfix and script_name..' {FFFFFF}SunFix {73b461}Enabled' or script_name..' {FFFFFF}SunFix {dc4747}Disabled', 0x73b461)
        gotofunc("SunFix")
        return false
    elseif cmd:find("^"..ini.commands.targetblip.."$") then
        ini.settings.targetblip = not ini.settings.targetblip
        save()
        sampAddChatMessage(ini.settings.targetblip and script_name..' {FFFFFF}TagetBlip {73b461}Enabled' or script_name..' {FFFFFF}TargetBlip {dc4747}Disabled', 0x73b461)
        gotofunc("TargetBlip")
        return false
    elseif cmd:find("^"..ini.commands.vsync.."$") then
        ini.settings.vsync = not ini.settings.vsync
        save()
        sampAddChatMessage(ini.settings.vsync and script_name..' {FFFFFF}Vsync {73b461}Enabled' or script_name..' {FFFFFF}Vsync {dc4747}Disabled', 0x73b461)
        gotofunc("Vsync")
        return false
    elseif cmd:find("^"..ini.commands.radarfix.."$")then
        ini.settings.radarfix = not ini.settings.radarfix
        save()
        sampAddChatMessage(ini.settings.radarfix and script_name..' {FFFFFF}RadarFix {73b461}Enabled' or script_name..' {FFFFFF}RadarFix {dc4747}Disabled', 0x73b461)
        gotofunc("Radarfix")
        return false
    elseif cmd:find("^"..ini.commands.radar_color_fix.."$")then
        ...
        sampAddChatMessage(ini.settings.radar_color_fix and script_name..' {FFFFFF}Radar Outline Color Fix {73b461}Enabled' or script_name..' {FFFFFF}Radar Outline Color Fix {dc4747}Disabled', 0x73b461)
    ...
    elseif cmd:find("^"..ini.commands.dual_monitor_fix.."$")then
        ...
        sampAddChatMessage(ini.settings.dual_monitor_fix and script_name..' {FFFFFF}DualMonitorFix {73b461}Enabled' or script_name..' {FFFFFF}DualMonitorFix {dc4747}Disabled', 0x73b461)
        return false
    elseif cmd:find("^"..ini.commands.adkfix.."$")then
        ...
        sampAddChatMessage(ini.settings.adkfix and script_name..' {FFFFFF}Ninja Hijack Fix {73b461}Enabled' or script_name..' {FFFFFF}Ninja Hijack Fix {dc4747}Disabled', 0x73b461)
        return false
    elseif cmd:find("^"..ini.commands.fixtaxilight.."$")then
        ...
        sampAddChatMessage(ini.settings.fixtaxilight and script_name..' {FFFFFF}TaxiLightFix {73b461}Enabled' or script_name..' {FFFFFF}TaxiLightFix {dc4747}Disabled', 0x73b461)
    ...
    elseif cmd:find("^"..ini.commands.radarWidth.." .+") or cmd:find("^"..ini.commands.radarWidth.."$")then
        ...
        if type(param) ~= 'number' then
            sampAddChatMessage(script_name.." {FFFFFF}Use: {dc4747}"..ini.commands.radarWidth.." [число]", 0x73b461)
            ...
            gotofunc("Radarfix")
            sampAddChatMessage(script_name.." {FFFFFF}Radar Height is set to: {dc4747}"..ini.settings.radarWidth, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.radarHeight.." .+") or cmd:find("^"..ini.commands.radarHeight.."$")then
        ...
        param = tonumber(param)
        if type(param) ~= 'number' then
            sampAddChatMessage(script_name.." {FFFFFF}Use: {dc4747}"..ini.commands.radarHeight.." [число]", 0x73b461)
            ...
            gotofunc("Radarfix")
            sampAddChatMessage(script_name.." {FFFFFF}Radar Witch is set to: {dc4747}"..ini.settings.radarHeight, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.radarx.." .+") or cmd:find("^"..ini.commands.radarx.."$")then
        ...
        param = tonumber(param)
        if type(param) ~= 'number' then
            sampAddChatMessage(script_name.." {FFFFFF}Use: {dc4747}"..ini.commands.radarx.." [число]", 0x73b461)
            ...
            gotofunc("Radarfix")
            sampAddChatMessage(script_name.." {FFFFFF}Radar X Position is set to: {dc4747}"..ini.settings.radarPosX, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.radary.." .+") or cmd:find("^"..ini.commands.radary.."$")then
        ...
        param = tonumber(param)
        if type(param) ~= 'number' then
            sampAddChatMessage(script_name.." {FFFFFF}Use: {dc4747}"..ini.commands.radary.." [число]", 0x73b461)
            ...
            gotofunc("Radarfix")
            sampAddChatMessage(script_name.." {FFFFFF}Radar Y Position is set to: {dc4747}"..ini.settings.radarPosY, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.ugenrl.."$")then
        ...
        sampAddChatMessage(ini.ugenrl_main.enable and script_name.." {FFFFFF}Ultimate Genrl {73b461}Enabled" or script_name.." {FFFFFF}Ultimate Genrl {dc4747}Disabled", 0x73b461)
        return false
    elseif cmd:find("^"..ini.commands.autonosounds.."$")then
        ...
        save()
        sampAddChatMessage(ini.ugenrl_main.autonosounds and script_name.." {FFFFFF}Auto Disabled Sounds Was {73b461}Enabled" or script_name.." {FFFFFF}Auto Disabled Sounds Was {dc4747}Disabled", 0x73b461)
        return false
    elseif cmd:find("^"..ini.commands.uds.." .+") or cmd:find("^"..ini.commands.uds.."$")then
        ...
        if type(param) ~= 'number' or param < 1 or param > tonumber(#deagleSounds) then
            ...
            save()
            sampAddChatMessage(script_name.." {FFFFFF}Deagle Sound set to: {dc4747}"..param, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.uss.." .+") or cmd:find("^"..ini.commands.uss.."$")then
        ...
        if type(param) ~= 'number' or param < 1 or param > tonumber(#shotgunSounds) then
            ...
            save()
            sampAddChatMessage(script_name.." {FFFFFF}Shotgun Sound set to: {dc4747}"..param, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.ums.." .+") or cmd:find("^"..ini.commands.ums.."$")then
        ...
        if type(param) ~= 'number' or param < 1 or param > tonumber(#m4Sounds) then
            ...
            save()
            sampAddChatMessage(script_name.." {FFFFFF}M4 Sound set to: {dc4747}"..param, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.urs.." .+") or cmd:find("^"..ini.commands.urs.."$")then
        ...
        if type(param) ~= 'number' or param < 1 or param > tonumber(#rifleSounds) then
            ...
            save()
            sampAddChatMessage(script_name.." {FFFFFF}Sniper Sound set to: {dc4747}"..param, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.uuzi.." .+") or cmd:find("^"..ini.commands.uuzi.."$")then
        ...
        if type(param) ~= 'number' or param < 1 or param > tonumber(#uziSounds) then
            ...
            save()
            sampAddChatMessage(script_name.." {FFFFFF}Uzi Sound Set to: {dc4747}"..param, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.ump5.." .+") or cmd:find("^"..ini.commands.ump5.."$")then
        ...
        if type(param) ~= 'number' or param < 1 or param > tonumber(#mp5Sounds) then
            ...
            save()
            sampAddChatMessage(script_name.." {FFFFFF}Mp5 Sound Set to: {dc4747}"..param, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.ubs.." .+") or cmd:find("^"..ini.commands.ubs.."$")then
        ...
        if type(param) ~= 'number' or param < 1 or param > tonumber(#hitSounds) then
            ...
            save()
            sampAddChatMessage(script_name.." {FFFFFF}Hitsound Set to: {dc4747}"..param, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.ups.." .+") or cmd:find("^"..ini.commands.ups.."$")then
        ...
        if type(param) ~= 'number' or param < 1 or param > tonumber(#painSounds) then
            ...
            save()
            sampAddChatMessage(script_name.." {FFFFFF}PainSound set to: {dc4747}"..param, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.ugd.." .+") or cmd:find("^"..ini.commands.ugd.."$")then
        ...
        if type(param) ~= 'number' or param < 0 or param > 100 then
            ...
            save()
            sampAddChatMessage(script_name.." {FFFFFF}Enemy Weapon Sound Dist set to: {dc4747}"..ini.ugenrl_main.enemyWeaponDist, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.ugvw.." .+") or cmd:find("^"..ini.commands.ugvw.."$")then
        ...
        if type(param) ~= 'number' or param < 0.00 or param > 1.00 then
            ...
            save()
            sampAddChatMessage(script_name.." {FFFFFF}Shots Sound volume Set to: {dc4747}"..ini.ugenrl_volume.weapon, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.ugvh.." .+") or cmd:find("^"..ini.commands.ugvh.."$")then
        ...
        if type(param) ~= 'number' or param < 0.00 or param > 1.00 then
            ...
            save()
            sampAddChatMessage(script_name.." {FFFFFF}HitSound Volume set to: {dc4747}"..ini.ugenrl_volume.hit, 0x73b461)
        end
        return false
    elseif cmd:find("^"..ini.commands.ugvp.." .+") or cmd:find("^"..ini.commands.ugvp.."$")then
        ...
        if type(param) ~= 'number' or param < 0.00 or param > 1.00 then
            ...
            save()
            sampAddChatMessage(script_name.." {FFFFFF}PainSound Volume set to: {dc4747}"..ini.ugenrl_volume.pain, 0x73b461)
	    end 
        return false
    elseif cmd:find("^"..ini.commands.forceaniso.."$")then
        ini.settings.forceaniso = not ini.settings.forceaniso
        save()
        sampAddChatMessage(ini.settings.forceaniso and script_name..' {FFFFFF}Anisotropic texture filtering {73b461}Enabled' or script_name..' {FFFFFF}Anisotropic texture filtering {dc4747}Disabled', 0x73b461)
        gotofunc("ForceAniso")
        return false
    elseif cmd:find("^"..ini.commands.mapzoomfixer.."$")then
        ...
        save()
        sampAddChatMessage(ini.settings.mapzoomfixer and script_name..' {FFFFFF}MapZoomFix {73b461}Enabled' or script_name..' {FFFFFF}MapZoomFix {dc4747}Disabled', 0x73b461)
        return false
    elseif cmd:find("^"..ini.commands.shadowcp.." .+") or cmd:find("^"..ini.commands.shadowcp.."$")then
        ...
        if ini.settings.shadowedit then
            if type(param) ~= 'number' or param < 0 or param > 255 then
                ...
                gotofunc("ShadowEdit")
                sampAddChatMessage(script_name.." {FFFFFF}ShadowCp set to: {dc4747}"..ini.settings.shadowcp, 0x73b461)
            end
        else
    ...
    elseif cmd:find("^"..ini.commands.shadowlight.." .+") or cmd:find("^"..ini.commands.shadowlight.."$")then
        ...
        if ini.settings.shadowedit then
            if type(param) ~= 'number' or param < 0 or param > 255 then
                ...
                gotofunc("ShadowEdit")
                sampAddChatMessage(script_name.." {FFFFFF}ShadowLight set to: {dc4747}"..ini.settings.shadowlight, 0x73b461)
            end
        else
            sampAddChatMessage(script_name.." {FFFFFF}Ability to Edit Shadows is disabled, Enable it: {dc4747}"..ini.commands.shadowedit, 0x73b461)
        end
...
function hpt()
	if ini.hphud.style == 1 then
        sampAddChatMessage(script_name.." {FFFFFF} HPStyle set to: {DC4747}Without \"hp\"", 0x73b461)
		ini.hphud.style = 0
        save()
	else
		sampAddChatMessage(script_name.." {FFFFFF} HPStyle set to: {DC4747}without \"hp\"", 0x73b461)
		...
		imgui.Begin("GameFixer", main_menu, imgui.WindowFlags.NoCollapse)
                ...
                imgui.BeginChild("##Main", imgui.ImVec2(-1, -1), true)
                    ...
                    if imgui.CollapsingHeader("HP HUD", true, imgui.TreeNodeFlags.DefaultOpen) then
                        if imgui.Button((ini.hphud.active and 'Disable' or 'Enable').." Digital Health Indicator", imgui.ImVec2(300, 25)) then
                            ini.hphud.active = not ini.hphud.active
                            sampAddChatMessage(ini.hphud.active and '{73b461}'..script_name..' {FFFFFF}Digital Health Indicator {73b461}Enabled' or '{73b461}'..script_name..' {FFFFFF}Digital Health Indicator {dc4747}Disabled', -1)
                            save()
                            if ini.hphud.active == false then
                                sampTextdrawDelete(2029)
                            end
                    ...
                    if imgui.Button((ini.settings.bighpbar and 'Disable' or 'Enable').." 160 HP Bar", imgui.ImVec2(200, 25)) then
                        ini.settings.bighpbar = not ini.settings.bighpbar
                        sampAddChatMessage(ini.settings.bighpbar and script_name..' {FFFFFF}160hp Bar {73b461}Enabled' or script_name..' {FFFFFF}160hp Bar {dc4747}Disabled', 0x73b461)
                        save()
                        gotofunc("BigHPBar")
                    end
                    imgui.SameLine()
                    if imgui.Button((ini.settings.noradio and 'Disable' or 'Enable').." Radio", imgui.ImVec2(200, 25)) then
                        ini.settings.noradio = not ini.settings.noradio
                        sampAddChatMessage(ini.settings.noradio and script_name..' {FFFFFF}Radio {73b461}Enabled' or script_name..' {FFFFFF}Radio {dc4747}Disabled', 0x73b461)
                        save()
                        gotofunc("NoRadio")
                    end
                    if imgui.Button((ini.settings.nosounds and 'Disable' or 'Enable').." Sound", imgui.ImVec2(200, 25)) then
                        ini.settings.nosounds = not ini.settings.nosounds
                        sampAddChatMessage(ini.settings.nosounds and script_name..' {FFFFFF}Sound {73b461}Enabled' or script_name..' {FFFFFF}Sound {dc4747}Disabled', 0x73b461)
                        save()
                        gotofunc("NoSounds")
                    end
                    imgui.SameLine()
                    if imgui.Button(u8(ini.settings.intmusic and 'Disable' or 'Enable').." InteriorMusic", imgui.ImVec2(200, 25)) then
                        ini.settings.intmusic = not ini.settings.intmusic
                        sampAddChatMessage(ini.settings.intmusic and script_name..' {FFFFFF}InteriorMusic {73b461}Enabled' or script_name..' {FFFFFF}InteriorMusic {dc4747}Disabled', 0x73b461)
                        save()
                        gotofunc("InteriorMusic")
                    end
                    ...
                    if imgui.Button(u8(ini.settings.targetblip and 'Disable' or 'Enable').." TargetBlip", imgui.ImVec2(413, 25)) then
                        ini.settings.targetblip = not ini.settings.targetblip
                        sampAddChatMessage(ini.settings.targetblip and script_name..' {FFFFFF}TargetBlip {73b461}Enabled' or script_name..' {FFFFFF}TargetBlip {dc4747}Disabled', 0x73b461)
                        save()
                        gotofunc("TargetBlip")
                    end
                    if imgui.Button((ini.settings.blockkeys and 'Disable' or 'Enable').." Samp Key Lock", imgui.ImVec2(413, 25)) then
                        ini.settings.blockkeys = not ini.settings.blockkeys
                        sampAddChatMessage(ini.settings.blockkeys and script_name..' {FFFFFF}Samp Key Lock {73b461}Enabled' or script_name..' {FFFFFF}Samp Key Lock {dc4747}Disabled', 0x73b461)
                        save()
                        gotofunc("BlockSampKeys")
                    end
                    if imgui.Button(u8(ini.settings.vsync and 'Disable' or 'Enable').." Vsync", imgui.ImVec2(413, 25)) then
                        ini.settings.vsync = not ini.settings.vsync
                        sampAddChatMessage(ini.settings.vsync and script_name..' {FFFFFF}Vsync {73b461}Enabled' or script_name..' {FFFFFF}Vsync {dc4747}Disabledа', 0x73b461)
                        save()
                        gotofunc("Vsync")
                    end
                imgui.EndChild()
            elseif tab.v == 2 then
                imgui.BeginChild("##Fixes", imgui.ImVec2(-1, -1), true)
                    ...
                    if imgui.Checkbox("Chat Opening on E", checkboxes.chatt) then
            ...
            elseif tab.v == 3 then
                imgui.BeginChild("##FPSBoost", imgui.ImVec2(-1, -1), true)
                    if imgui.Button(u8(ini.settings.shownicks and 'Show' or 'Hide').." NickNames", imgui.ImVec2(200, 25)) then
                        ini.settings.shownicks = not ini.settings.shownicks
                        sampAddChatMessage(ini.settings.shownicks and script_name..' {FFFFFF}NickNames {dc4747}Disabled' or script_name..' {FFFFFF}NickNames {73b461}Enabled', 0x73b461)
                        save()
                        gotofunc("ShowNICKS")
                    end
                    imgui.SameLine()
                    if imgui.Button(u8(ini.settings.showhp and 'Show' or 'Hide').." PlayerHP", imgui.ImVec2(200, 25)) then
                        ini.settings.showhp = not ini.settings.showhp
                        sampAddChatMessage(ini.settings.showhp and script_name..' {FFFFFF}PlayerHP {dc4747}Disabled' or script_name..' {FFFFFF}PlayerHP {73b461}Enabled', 0x73b461)
                        save()
                        gotofunc("ShowHP")
                    end
                    imgui.SameLine()
                    if imgui.Button(u8(ini.settings.showchat and 'Hide' or 'Show').." Chat", imgui.ImVec2(200, 25)) then
                        ini.settings.showchat = not ini.settings.showchat
                        sampAddChatMessage(ini.settings.showchat and script_name..' {FFFFFF}Chat {73b461}UnHidden' or script_name..' {FFFFFF}Chat {dc4747}Hidden', 0x73b461)
                        save()
                        gotofunc("ShowCHAT")
                    end
                    if imgui.Button(u8(ini.settings.showhud and 'Hide' or 'Show').." HUD", imgui.ImVec2(200, 25)) then
                        ini.settings.showhud = not ini.settings.showhud
                        sampAddChatMessage(ini.settings.showhud and script_name..' {FFFFFF}HUD {73b461}UnHidden' or script_name..' {FFFFFF}HUD {dc4747}Hidden', 0x73b461)
                        save()
                        gotofunc("ShowHUD")
                    end
                    imgui.SameLine()
                    if imgui.Button(u8(ini.settings.unlimitfps and 'Disable' or 'Enable').." FPS unlock", imgui.ImVec2(200, 25)) then
                        ini.settings.unlimitfps = not ini.settings.unlimitfps
                        sampAddChatMessage(ini.settings.unlimitfps and script_name..' {FFFFFF}FPS Unlock {73b461}Enabled' or script_name..' {FFFFFF}FPS Unlock {dc4747}Disabled', 0x73b461)
                        save()
                        gotofunc("FPSUnlock")
                    end
                    imgui.SameLine()
                    if imgui.Button(u8(ini.settings.postfx and 'Disable' or 'Enable').." Postfx", imgui.ImVec2(200, 25)) then
                        ini.settings.postfx = not ini.settings.postfx
                        sampAddChatMessage(ini.settings.postfx and script_name..' {FFFFFF}Postfx {73b461}Enabled' or script_name..' {FFFFFF}Postfx {dc4747}Disabled', 0x73b461)
                        save()
                        gotofunc("NoPostfx")
                    end
                    if imgui.Button(u8(ini.settings.nocloudsmall and 'Disable' or 'Enable').." Low Clouds", imgui.ImVec2(200, 25)) then
                        ini.settings.nocloudsmall = not ini.settings.nocloudsmall
                        save()
                        sampAddChatMessage(ini.settings.nocloudsmall and script_name..' {FFFFFF}Low Clouds {73b461}Enabled' or script_name..' {FFFFFF}Low Clouds {dc4747}Disabled', 0x73b461)
                        gotofunc("NoCloudSmall")
                    end
                    imgui.SameLine()
                    if imgui.Button(u8(ini.settings.nocloudbig and 'Disable' or 'Enable').." High Clouds", imgui.ImVec2(200, 25)) then
                        ini.settings.nocloudbig = not ini.settings.nocloudbig
                        save()
                        sampAddChatMessage(ini.settings.nocloudbig and script_name..' {FFFFFF}High Clouds {73b461}Enabled' or script_name..' {FFFFFF}High Clouds {dc4747}Disabled', 0x73b461)
                        gotofunc("NoCloudBig")
                    end
                    imgui.SameLine()
                    if imgui.Button(u8(ini.settings.nobirds and 'Disable' or 'Enable').." Birds", imgui.ImVec2(200, 25)) then
                        ini.settings.nobirds = not ini.settings.nobirds
                        save()
                        sampAddChatMessage(ini.settings.nobirds and script_name..' {FFFFFF}Birds {73b461}Enabled' or script_name..' {FFFFFF}Birds {dc4747}Disabled', 0x73b461)
                        gotofunc("NoBirds")
                    end
                    if imgui.Button(u8(ini.settings.effects and 'Disable' or 'Enable').." Effects", imgui.ImVec2(200, 25)) then
                        ini.settings.effects = not ini.settings.effects
                        save()
                        sampAddChatMessage(ini.settings.effects and script_name..' {FFFFFF}Effects {73b461}Enabled' or script_name..' {FFFFFF}Effects {dc4747}Disabled', 0x73b461)
                        gotofunc("NoEffects")
                    end
                    imgui.SameLine()
                    if imgui.Button(u8(ini.settings.vehlods and 'Disable' or 'Enable').." VehLods", imgui.ImVec2(200, 25)) then
                        ini.settings.vehlods = not ini.settings.vehlods
                        save()
                        sampAddChatMessage(ini.settings.vehlods and script_name..' {FFFFFF}VehLods {73b461}Enabled' or script_name..' {FFFFFF}VehLods {dc4747}Disabled', 0x73b461)
                        gotofunc("VehLods")
                    end
                    imgui.SameLine()
                    if imgui.Button(u8(ini.settings.noshadows and 'Disable' or 'Enable').." Shadows", imgui.ImVec2(200, 25)) then
                        ini.settings.noshadows = not ini.settings.noshadows
                        save()
                        sampAddChatMessage(ini.settings.noshadows and script_name..' {FFFFFF}Shadows {73b461}Enabled' or script_name..' {FFFFFF}Shadows {dc4747}Disabled', 0x73b461)
                        gotofunc("NoShadows")
                    end
                    if imgui.Button(u8(ini.settings.nodust and 'Disable' or 'Enable').." Dust", imgui.ImVec2(300, 25)) then
                        ini.settings.nodust = not ini.settings.nodust
                        save()
                        sampAddChatMessage(ini.settings.nodust and script_name..' {FFFFFF}Dust {73b461}Enabled' or script_name..' {FFFFFF}Dust {dc4747}Disabled', 0x73b461)
                        gotofunc("NoDust")
                    end
                    imgui.SameLine()
                    if imgui.Button(u8(ini.settings.noplaneline and 'Disable' or 'Enable').." Plane Lines", imgui.ImVec2(300, 25)) then
                        ini.settings.noplaneline = not ini.settings.noplaneline
                        save()
                        sampAddChatMessage(ini.settings.noplaneline and script_name..' {FFFFFF}Plane Lines {73b461}Enabled' or script_name..' {FFFFFF}Plane Lines  {dc4747}Disabled', 0x73b461)
                        gotofunc("NoPlaneLine")
                    end
                    if imgui.CollapsingHeader("Distance Editor") then
                        if imgui.Button(u8(ini.settings.givemedist and 'Disable' or 'Enable').." Ability to Change Distance", imgui.ImVec2(300, 25)) then
                            ini.settings.givemedist = not ini.settings.givemedist
                            sampAddChatMessage(ini.settings.givemedist and script_name..' {FFFFFF}Ability to Change Distance {73b461}Enabled' or script_name..' {FFFFFF}Ability to Change Distance {dc4747}Disabledа', 0x73b461)
                            save()
                            gotofunc("GivemeDist")
                        end
                        if ini.settings.givemedist then
                    ...
                    if imgui.CollapsingHeader("Shadow Editor") then
                        if imgui.Button(u8(ini.settings.shadowedit and 'Disable' or 'Enable').." Ability to Edit Shadows", imgui.ImVec2(300, 25)) then
                            ini.settings.shadowedit = not ini.settings.shadowedit
                            sampAddChatMessage(ini.settings.shadowedit and script_name..' {FFFFFF}Ability to Edit Shadows {73b461}Enabled' or script_name..' {FFFFFF}Ability to Edit Shadows {dc4747}Disabled', 0x73b461)
                            save()
                            gotofunc("ShadowEdit")
                        end
                        if ini.settings.shadowedit then
                    ...
                    if imgui.CollapsingHeader("Timecycle Fix") then
                        if imgui.Button((ini.fixtimecyc.active and 'Disable' or 'Enable').." Timecycle Fix", imgui.ImVec2(300, 25)) then
                            ini.fixtimecyc.active = not ini.fixtimecyc.active
                            sampAddChatMessage(ini.fixtimecyc.active and script_name..' {FFFFFF}Timecycle Fix {73b461}Enabled' or script_name..' {FFFFFF}Timecycle Fix {dc4747}Disabled', 0x73b461)
                            save()
                            gotofunc("FixTimecyc")
                        end
                    ...
                    if imgui.CollapsingHeader("CCL") then
                        ...
                        if imgui.Button(u8(ini.cleaner.autoclean and 'Disable' or 'Enable').." CCl", imgui.ImVec2(300, 25)) then
                            ini.cleaner.autoclean = not ini.cleaner.autoclean
                            save()
                            sampAddChatMessage((script_name.."{FFFFFF} CCl %s"):format(ini.cleaner.autoclean and "{73b461}Enabled" or "{dc4747}Disabled"), 0x73b461)
                        end
            ...
            elseif tab.v == 4 then
                imgui.BeginChild("##Ugenrl", imgui.ImVec2(-1, -1), true)
                    ...
                    if checkboxes.ugenrl_enable.v then
                        ...
                        if imgui.HotKey(u8"##ugkey", FastUgenrlKey, tLastKeys, 70) then
                            ...
                            sampAddChatMessage(script_name.." {FFFFFF}Old Keys: {dc4747}" .. table.concat(rkeys.getKeysName(tLastKeys.v), " + ") .. "{ffffff} | New Keys: {dc4747}" .. table.concat(rkeys.getKeysName(FastUgenrlKey.v), " + "), 0x73b461)
                ...
                imgui.EndChild()
            elseif tab.v == 5 then
                imgui.BeginChild("##Команды", imgui.ImVec2(-1, -1), true)
                    imgui.PushItemWidth(130)
                    if imgui.InputText("Command to open menu", buffers.cmd_openmenu) then
                        ini.commands.openmenu = buffers.cmd_openmenu.v
                        save()
                    end
                    if imgui.InputText("Change Time", buffers.cmd_settime) then
                        ini.commands.settime = buffers.cmd_settime.v
                        save()
                    end
                    if imgui.InputText("Change Weather", buffers.cmd_setweather) then
                        ini.commands.setweather = buffers.cmd_setweather.v
                        save()
                    end
                    if imgui.InputText("Freeze Time", buffers.cmd_blockservertime) then
                        ini.commands.blockservertime = buffers.cmd_blockservertime.v
                        save()
                    end
                    if imgui.InputText("Freeze Weather", buffers.cmd_blockserverweather) then
                    ...
                    if imgui.InputText("Enable / Disable High/Big Clouds", buffers.cmd_nocloudbig) then
                        ini.commands.nocloudbig = buffers.cmd_nocloudbig.v
                        save()
                    end
                    if imgui.InputText("Enable / Disable Low/Small Clouds", buffers.cmd_nocloudsmall) then
                    ...
                    if imgui.InputText("Enable / Disable Samp Key Lock", buffers.cmd_blocksampkeys) then
...
function onWindowMessage(msg, wparam, lparam)
    ...
    if msg == 261 and wparam == 13 then --запрет на alt+enter
        consumeWindowMessage(true, true)
    end
end
...
function gotofunc(fnc)
    ...
    if fnc == "AudioStream" or fnc == "all" then
        if sampGetVersion() == "0.3.7-R1" then
            if ini.settings.audiostream then
                memory.write(sampGetBase() + 104848, 9449, 2, true)-- Onючает аудиострим
            else
                memory.write(sampGetBase() + 104848, 50064, 2, true)-- Offючает аудиострим
            end
        end
	end
    ...
    if fnc == "CleanMemory" then
        ...
        if ini.cleaner.cleaninfo then
            sampAddChatMessage(script_name.."{FFFFFF} Clean Memory up to: {dc4747}"..oldram.." МB. {FFFFFF}Memory After: {dc4747}"..newram.." MB. {FFFFFF}Cleared: {dc4747}"..oldram - newram.." МB.", 0x73b461)
        end
    end
    ...
    if fnc == "Radarfix" or fnc == "all" then
        if ini.settings.radarfix then
            memory.setfloat(8809336, ini.settings.radarHeight, false)--высота радара X
            memory.setfloat(8809332, ini.settings.radarWidth, false)--высота радара Y
            memory.setfloat(8751632, ini.settings.radarPosX, false)--позиция радара по X
            memory.setfloat(8809328, ini.settings.radarPosY, false)--позиция радара по Y
            ...
            memory.setfloat(8809336, 94.0, false)--высота радара X
            memory.setfloat(8809332, 76.0, false)--высота радара Y
            memory.setfloat(8751632, 40.0, false)--позиция радара по X
            memory.setfloat(8809328, 104.0, false)--позиция радара по Y
        end
